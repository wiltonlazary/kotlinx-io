...
class JsonInputOutputRecursiveTest : JsonTestBase() {
    ...
    fun testWriteDataString() = parametrizedTest { useStreaming ->
        val outputData = Event(
            0,
            DummyEither
                .Right(Payload(42, 43, "Hello world")),
            1000
        )
    ...
    fun testWriteDataStringIndented() = parametrizedTest { useStreaming ->
        ...
        val ev = ioJson
            .indented.stringify(Event.serializer(), outputData, useStreaming)
        assertEquals(
            """{
            |    "id": 0,
            |    "payload": {
    ...
    fun testWriteErrorString() = parametrizedTest { useStreaming ->
        val outputError = Event(
            1,
            DummyEither.Left("Connection timed out"),
            1001
        )
    ...
    private object EitherSerializer : KSerializer<DummyEither> {
        ...
        override fun deserialize(decoder: Decoder): DummyEither {
            ...
            if ("error" in tree) return DummyEither
                .Left(tree.getPrimitive("error").content)
            return DummyEither.Right(
                decoder.json.fromJson(
                    Payload.serializer(),
                    tree
                )
            )
    ...
    private object RecursiveSerializer : KSerializer<DummyRecursive> {
        ...
        override fun serialize(encoder: Encoder, obj: DummyRecursive) {
            ...
            val (tree, typeName) = when (obj) {
                is DummyRecursive.A -> encoder.json.toJson(
                    DummyRecursive.A.serializer(), obj
                ) to typeNameA
                is DummyRecursive.B -> encoder.json.toJson(
                    DummyRecursive.B.serializer(), obj
                ) to typeNameB
            }
            val contents: MutableMap<String, JsonElement> = mutableMapOf(
                typeAttribute to JsonPrimitive(typeName)
            )
        ...
        override fun deserialize(decoder: Decoder): DummyRecursive {
            ...
            val tree = jsonReader.decodeJson() as? JsonObject
                ?: throw SerializationException("Expected JSON object")
            val typeName = tree.getValue(
                typeAttribute
            ).primitive.content
            val objTree = JsonObject(tree.content - typeAttribute
            )
            return when (typeName) {
                typeNameA -> decoder.json.fromJson(
                    DummyRecursive.A.serializer(), objTree
                )
                typeNameB -> decoder.json.fromJson(
                    DummyRecursive.B.serializer(), objTree
                )
