...
class JsonMapKeysTest : JsonTestBase() {
    ...
    fun structuredMapKeysShouldBeBannedByDefault() = parametrizedTest { streaming ->
        val e = assertFailsWith<JsonException> {
            ioJson(ioJsonConfiguration.Default).stringify(
    ...
    fun structuredMapKeysAllowedWithFlag() = assertJsonFormAndRestored(
        ...
        WithComplexKey(mapOf(IntData(42) to "42")),
        """{"map":[{"intV":42},"42"]}""",
        ioJson(ioJsonConfiguration.Default.copy(allowStructuredMapKeys = true))
    )
    ...
    fun enumsAreAllowedAsMapKeys() = assertJsonFormAndRestored(
        ...
        WithEnum(
            mapOf(
                SampleEnum.OptionA to 1L,
                SampleEnum.OptionC to 3L
            )
        ),
