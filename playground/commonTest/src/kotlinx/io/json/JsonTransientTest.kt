...
class JsonTransientTest : JsonTestBase() {
    ...
    fun testAll() = parametrizedTest { useStreaming ->
        assertEquals("{a:0,e:false,c:Hello}", unquoted.stringify(
            Data.serializer(),
            Data(), useStreaming))
    }
    ...
    fun testMissingOptionals() = parametrizedTest { useStreaming ->
        assertEquals(strict.parse(Data.serializer(), "{a:0,c:Hello}", useStreaming),
            Data()
        )
        assertEquals(strict.parse(Data.serializer(), "{a:0}", useStreaming),
            Data()
        )
    }
