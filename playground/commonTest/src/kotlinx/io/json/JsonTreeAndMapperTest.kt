...
object EitherSerializer : KSerializer<DummyEither> {
    ...
    override fun deserialize(decoder: Decoder): DummyEither {
        ...
        return DummyEither
            .Right(input.json.fromJson(Payload.serializer(), tree))
    }

    override fun serialize(encoder: Encoder, obj: DummyEither) {
        val output = encoder as? ioJsonOutput ?: throw SerializationException("This class can be saved only by Json")
        val tree = when (obj) {
...
class JsonTreeAndMapperTest {
    ...
    @Test
    fun testParseData() {
        val ev = ioJson.parse(Event.serializer(), decoderData)
        with(ev) {
            assertEquals(0, id)
            assertEquals(
                DummyEither
    ...
    @Test
    fun testParseError() {
        val ev = ioJson.parse(Event.serializer(), decoderError)
        with(ev) {
            assertEquals(1, id)
    ...
    @Test
    fun testWriteData() {
        val encoderData = Event(
            0,
            DummyEither
                .Right(Payload(42, 43, "Hello world")),
            1000
        )
        val ev = ioJson.stringify(Event.serializer(), encoderData)
    ...
    fun testWriteError() {
        ...
        val ev = ioJson.stringify(Event.serializer(), encoderError)
