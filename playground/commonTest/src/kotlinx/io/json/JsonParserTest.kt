...
class JsonParserTest {
    ...
    private fun parse(input: String) = ioJson.plain.parseJson(input).jsonObject
    ...
    val strict = ioJson(ioJsonConfiguration.Stable.copy(strictMode = true))
    ...
    fun testParseEscapedSymbols() {
        ...
        assertEquals(
            StringData("\"test\""), strict.parse(
                StringData.serializer(), """{"data": "\"test\""}"""))
        assertEquals(
            StringData("\u00c9"),
            strict.parse(
                StringData.serializer(), """{"data": "\u00c9"}"""
            )
        )
        assertEquals(
            StringData("""\\"""),
