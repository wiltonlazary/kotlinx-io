...
class JsonTreeTest : JsonTestBase() {
    ...
    private val json =
        ioJson(ioJsonConfiguration.Default)
    ...
    fun testReadTreeNullable() {
        ...
        assertEquals(
            DataWrapper("foo", null),
            json.fromJson(
                DataWrapper.serializer(), tree1
            )
        )
        assertFailsWith(MissingFieldException::class) { json.fromJson(
            DataWrapper.serializer(), tree2) }
    }
    ...
    fun testReadTreeOptional() {
        ...
        assertEquals(
            ...
            json.fromJson(
                DataWrapperOptional.serializer(), tree1
            )
        )
        assertEquals(
            DataWrapperOptional("foo", null),
            json.fromJson(
                DataWrapperOptional.serializer(), tree2
            )
        )
    }
    ...
    fun testReadTreeList() {
        ...
        assertEquals(
            ...
            json.fromJson(DataList.serializer(), tree2)
        )
        assertEquals(
            ListOfLists(listOf(listOf(), listOf(Data(42)))),
            json.fromJson(
                ListOfLists.serializer(), tree3
            )
        )
    }
    ...
    private inline fun <reified T : Any> writeAndTest(
        obj: T,
        serial: KSerializer<T>,
        printDiagnostics: Boolean = false
    ): Pair<JsonElement, T> {
    ...
    fun testSaveOptional() {
        writeAndTest(
            DataWrapperOptional("foo", null), DataWrapperOptional.serializer())
    }

    @Test
    fun testSaveAllTypes() {
        writeAndTest(
            AllTypes(1, -2, 100500, 0.0f, 2048.2, 'a', true, "foobar"), AllTypes.serializer())
    }
}
