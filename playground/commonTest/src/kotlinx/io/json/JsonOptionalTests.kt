...
class JsonOptionalTests : JsonTestBase() {
    ...
    fun testAll() = parametrizedTest { useStreaming ->
        assertEquals("{a:0,b:42,c:Hello}", unquoted.stringify(
            Data.serializer(),
            Data(), useStreaming))
        assertEquals(unquoted.parse(Data.serializer(), "{a:0,b:43,c:Hello}", useStreaming),
            Data(b = 43)
        )
        assertEquals(unquoted.parse(Data.serializer(), "{a:0,b:42,c:Hello}", useStreaming),
            Data()
        )
    }
    ...
    fun testMissingOptionals() = parametrizedTest { useStreaming ->
        assertEquals(unquoted.parse(Data.serializer(), "{a:0,c:Hello}", useStreaming),
            Data()
        )
        assertEquals(unquoted.parse(Data.serializer(), "{a:0}", useStreaming),
            Data()
        )
    }
